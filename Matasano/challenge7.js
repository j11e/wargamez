// this challenge uses Jeff Mott's library CryptoJS
// https://code.google.com/p/crypto-js/

function extractIV(ciphertext) {
    // the first 16 bytes of ciphertext in ASCII are the IV
    var ciphertext_ascii = base64_to_ascii(ciphertext);

    var iv = ciphertext_ascii.substring(0,16); 
    var actualCipherText = ciphertext_ascii.substring(16);

    return ascii_to_base64(iv);
}

function decryptAES_ECB(ciphertext, key, iv) {
    var decrypted;

    var plaintext = CryptoJS.AES.decrypt(
                            {
                                ciphertext: CryptoJS.enc.Base64.parse(ciphertext),
                                salt: ""
                            },
                            CryptoJS.enc.Hex.parse(key),
                            { 
                                mode: CryptoJS.mode.ECB,
                                iv: CryptoJS.enc.Base64.parse(iv) 
                            }
    );

    decrypted = hex_to_ascii(plaintext.toString());

    return decrypted;
}

displayMessage("Challenge number se7en", true);

var ch7_ciphertext = "";

var ch7_iv = extractIV(ch7_ciphertext);

var ch7_key = ascii_to_hex("YELLOW SUBMARINE");

displayMessage('Decrypting given ciphertext with key "YELLOW SUBMARINE": ');
displayMessage(decryptAES_ECB(ch7_ciphertext, ch7_key, ch7_iv));